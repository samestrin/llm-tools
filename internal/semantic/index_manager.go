...
func (m *IndexManager) Index(ctx context.Context, rootPath string, opts IndexOptions) (*IndexResult, error) {
	...
	for _, file := range files {
		...
		if err := m.processFile(ctx, file, result); err != nil {
...
func (m *IndexManager) processFile(ctx context.Context, filePath string, result *IndexResult) error {
	...
	for _, chunk := range chunks {
		...
		if err := m.storage.Create(ctx, chunk, embedding); err != nil {
...
func (m *IndexManager) fileNeedsUpdate(ctx context.Context, filePath string) (bool, error) {
	...
	chunks, err := m.storage.List(ctx, ListOptions{FilePath: filePath, Limit: 1})
	if err != nil {
		return true, nil // Assume needs update on error
	}

	if len(chunks) == 0 {
		return true, nil // No chunks = needs indexing
	}

	// Compare with stored hash (stored in chunk ID prefix)
	// For now, always update if file exists in index
	// A more sophisticated approach would store file hashes separately
	existingHash := ""
	if len(chunks[0].ID) >= 16 {
		existingHash = chunks[0].ID[:16]
	}

	return existingHash != currentHash[:16], nil
}
