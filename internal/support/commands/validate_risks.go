...
var (
	...
	// Risk ID pattern matches R-1, R-2, etc.
	riskIDPattern = regexp.MustCompile(`\bR-(\d+)\b`)
	// Risk section heading patterns
	riskSectionPattern = regexp.MustCompile(`(?i)^#+\s*(Risk\s*Analysis|Potential\s*Risks)\s*$`)
	// Table row pattern for risk extraction
	riskTableRowPattern = regexp.MustCompile(`\|\s*(R-\d+)[:\s]*([^|]+)`)
	// List format pattern for risks
	riskListPattern = regexp.MustCompile(`^[\s]*[-*+]\s+\*{0,2}(R-\d+)\*{0,2}[:\s]*(.*)`)
)
...
func runValidateRisks(cmd *cobra.Command, args []string) error {
	...
	coverage := findRiskCoverage(risks, validateRisksStories, validateRisksTasks, validateRisksAC)

	// Build result
	result := buildRiskResult(risks, coverage, validateRisksDesign)
...
func parseRisksFromDesign(filePath string) ([]RiskDetail, error) {
	...
	for i, line := range lines {
		...
		for _, match := range inlineMatches {
			// Only add if we haven't seen this risk ID yet
...
func findRiskCoverage(risks []RiskDetail, storiesDir, tasksDir, acDir string) map[string][]string {
	...
	if storiesDir != "" {
		searchDirForRisks(storiesDir, riskIDs, coverage)
	}

	// Search tasks directory
	if tasksDir != "" {
		searchDirForRisks(tasksDir, riskIDs, coverage)
	}

	// Search acceptance criteria directory
	if acDir != "" {
		searchDirForRisks(acDir, riskIDs, coverage)
	}
...
func searchDirForRisks(dir string, riskIDs map[string]bool, coverage map[string][]string) {
...
func buildRiskResult(risks []RiskDetail, coverage map[string][]string, designFile string) ValidateRisksResult {
	var addressed int
